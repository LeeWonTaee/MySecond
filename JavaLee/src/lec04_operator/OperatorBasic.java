...
public class OperatorBasic {
	public static void main(String[] args) {
		...
		++intVar; // 1 증가
		System.out.println(intVar); // intVar은 11
		
		System.out.println(intVar++);
		System.out.println(intVar); // intVar은 12
		...
		int numB = 3;
		numA = numA + 3;
		System.out.println(numA); // numAㄴ느 13
		...
		System.out.printf("현재 게시판에 총 %d개의 게시글이 있고 " + 
			"한 페이지에 보여줄 글의 수가 %d개 일 때,"
			+ "총 페이지의 수는 %d개  이며 마지막 페이지에서는  "
			+"%d개의 글이 보여집니다.\n", totalCountGul, countLastPageGul, countPage, countLastPageGul);
		
		// 문자열 더하기
		String subway = "반석";
		subway = subway + ", 지족";
		//String 타입에서 대해 + 기호는 이어붙이기 수식이다.
...
//		System.out.println(strB);
		...
		strC = "";
		String strCIsEmpty = (strC.isEmpty()) ? "비어있습니다." : "비어있지않습니다.";
		...
		// 논리 연산자를 쓰면 if문을 많이 만들지(if문 중첩) 않아도 된다.
		...
		System.out.println("& = " + (numTen & numNine)); // 이진수 1000 = 8
		System.out.println("| = " + (numTen | numNine)); // 이진수 1011 = 11
		System.out.println("^ = " + (numTen ^ numNine)); // 이진수 0011 = 3
		...
		/*
		 *  r => 읽기 w => 쓰기 x => 실행
		 *  ex) r w x => 1 1 1, r - x => 1 0 1
		 *  오너, 그룹, 일반인 rwxr-xr-x일시 7 7 5
		 */
