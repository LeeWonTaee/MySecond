...
public class Conditional {
	public static void main(String[] args) {
		...
		int intVar = 3;
		if(intVar > 0) {
			System.out.println("intVar이 0보다 크다");
		}
		
		// 블록{} 안 명령어가 한줄이면 블록을 생략해도 정상 작동
		if(intVar > 0) System.out.println("intVar이 0보다 크다");
		...
		// 전원 버튼을 눌렀을때의 로직
		if(isPowerOn) {	//isPowerOn가 ture면 TV가 켜져있다
			System.out.println("TV를 끕니다.");
			isPowerOn = false;
		} else {
			System.out.println("TV가 켭니다.");
			isPowerOn = true;
		}
		...
		// boolean의 true/false를 스위치에 on/off처럼 사용하는 
		...
		int month = 0; // 교육기간(단위 : 월)
		
		if(month == 0) {
			System.out.println("열심히 배우고있습니다.");
		}else if(month == 1) {
